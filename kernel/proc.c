...
// task 4.2 - counter for FCFS scheduler
uint64 next_runnable_time = 1;
struct spinlock runnable_time_lock;

extern void forkret(void);
static void freeproc(struct proc *p);
int check_run_time_ratio(struct proc *proc1, struct proc *proc2);
...
void procinit(void)
{
  ...
  // task 4.2 - initiate lock for runnable time
  initlock(&runnable_time_lock, "next_runnable");
...
uint64 get_runnable_time()
{
  uint64 runnable_time;

  acquire(&runnable_time_lock);
  runnable_time = next_runnable_time;
  next_runnable_time = next_runnable_time + 1;
  release(&runnable_time_lock);
...
int fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // task 4.2
  np->fcfs_time = get_runnable_time();
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        c->proc = p;
        acquire(&tickslock);
        p->srt_time = ticks; // task 4.2 - get running time for burst time computation
        release(&tickslock);
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        {
#ifdef FCFS
          sched_condition = p->fcfs_time < scheduled_process->fcfs_time;
#endif

#ifdef SRT
          sched_condition = p->performance.average_bursttime < scheduled_process->performance.average_bursttime;
#endif

#ifdef CFSD
          sched_condition = check_run_time_ratio(p, scheduled_process);
#endif
          if (sched_condition)
          {
...
int check_run_time_ratio(struct proc *proc1, struct proc *proc2)
{
  int decay_factor[5] = {1, 3, 5, 7, 25};

  int numerator_proc1 = proc1->performance.rutime * decay_factor[proc1->priority - 1];
  int numerator_proc2 = proc2->performance.rutime * decay_factor[proc2->priority - 1];
  int denominator_proc1 = proc1->performance.rutime + proc1->performance.stime;
  int denominator_proc2 = proc2->performance.rutime + proc2->performance.stime;

  if (denominator_proc1 == 0)
  {
    numerator_proc1 = 0;
    denominator_proc1 = 1;
  }

  if (denominator_proc2 == 0)
  {
    numerator_proc2 = 0;
    denominator_proc2 = 1;
  }

  return numerator_proc1 * denominator_proc2 < numerator_proc2 * denominator_proc1;
}
