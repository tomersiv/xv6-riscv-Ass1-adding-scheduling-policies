...
void runcmd(struct cmd *cmd)
{
  ...
  case EXEC:
    ...
    //task 1 - reading path file
    int fd_path;
    fd_path = open("/path", O_RDONLY);
    if (fd_path < 0)
    {
      fprintf(2, "Error in opening PATH file");
      break;
    }
    char *buff = (char *)malloc((100 * sizeof(char)));
    if (read(fd_path, buff, 100) < 0)
    {
      fprintf(2, "Error in reading PATH file");
      break;
    }
    close(fd_path);
    char delim[] = ":";
    char *ptr = str_split(buff, delim);
    free(buff);

    // task 1 - trying to execute from each directory in path file
    while (ptr != NULL)
    {
      exec(concatenate_strings(ptr, ecmd->argv[0]), ecmd->argv);
      ptr = str_split(NULL, delim);
    }
...
char *concatenate_strings(char *strg1, char *strg2)
    {
      char *start = strg1;
...
char *str_split(char *str, char *comp)
{
  static int pos;
  static char *s;
  int start = pos;
